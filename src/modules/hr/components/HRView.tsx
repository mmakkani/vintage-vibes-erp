...
export const HRView: React.FC<HRViewProps> = ({ onRefreshAll }) => {
  ...
  const loadSheetWindowData = async (month: string, forceSync: boolean = false) => {
    try {
      ...
      if (!isPosted || forceSync) {
        ...
        const existingEmpIds = new Set(records.map(a => String(a.employeeId || (a as any).employee_id || '')));
        const existingCodes = new Set(records.map(a => String(a.empCode || (a as any).emp_code || '').trim().toLowerCase()));

        const missing = activeEmps.filter((emp: any) => {
          const idStr = String(emp.id);
          const codeStr = String(emp.empCode || emp.code || emp.employee_code || '').trim().toLowerCase();
        ...
        if (missing.length > 0) {
          const newRows: AttendanceRecord[] = missing.map((emp: any) => ({
            id: `att-${String(emp.id)}-${month}`,
            employeeId: String(emp.id),
            employeeName: (emp.name || emp.fullName || `${emp.first_name || ''} ${emp.last_name || ''}`).trim() || 'Staff Member',
            empCode: emp.empCode || emp.code || emp.employee_code || '',
            monthYear: month,
            daysWorked: 30,
            overtimeHours: 0,
            status: 'DRAFT',
  ...
  return (
    <div className="space-y-3">
      ...
      {activeAttendanceSheetMonth && (
        <AttendanceModal
          isOpen={!!activeAttendanceSheetMonth}
          month={activeAttendanceSheetMonth}
          onClose={() => setActiveAttendanceSheetMonth(null)}
          employees={employees}
          attendance={sheetWindowAttendance}
          onUpdateAttendance={handleUpdateWindowAttendance}
          onSetAllDays={handleWindowSetAllDays}
          onPostSheet={handleWindowPostAttendanceSheet}
          onUnpostSheet={handleWindowUnpostAttendanceSheet}
          onSyncMissing={async () => {
            if (activeAttendanceSheetMonth) {
              await loadSheetWindowData(activeAttendanceSheetMonth, true);
              showMsg('Synced active employees to attendance roster!');
            }
          }}
          onSaveDraft={() => showMsg(`Attendance sheet for ${activeAttendanceSheetMonth} saved in DRAFT.`)}
          isPosted={isWindowSheetPosted}
        />
      )}
