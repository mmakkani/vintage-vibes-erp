...
export const SetupView: React.FC<SetupViewProps> = ({ onRefreshAll }) => {
  const [subTab, setSubTabState] = useState<SetupSubTab>(() => {
    try {
      ...
      if (sub && ['profile', 'payment_gateways', 'live_multicast_sockets', 'banks', 'pos_terminal', 'currency', 'categories', 'sizes', 'items', 'brands', 'labels', 'shops', 'whatsapp', 'bale_qr', 'security'].includes(sub)) {
        return sub;
      }
      ...
      if (saved && ['profile', 'payment_gateways', 'live_multicast_sockets', 'banks', 'pos_terminal', 'currency', 'categories', 'sizes', 'items', 'brands', 'labels', 'shops', 'whatsapp', 'bale_qr', 'security'].includes(saved)) {
        return saved;
      }
    } catch {}
    return 'profile';
  ...
  const [boothConfigs, setBoothConfigs] = useState<LiveBoothStreamConfig[]>([
    ...
    {
      boothId: 'booth-3',
      boothName: 'Booth 3: Branded Tees & Vintage Sportswear',
