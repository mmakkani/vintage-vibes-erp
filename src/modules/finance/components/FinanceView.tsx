...
export const FinanceView: React.FC<FinanceViewProps> = ({ onRefreshAll, currentUserRole, initialSubTab = 'coa', maintenanceModules, companyProfile }) => {
  ...
  const handleDeleteVoucher = async (v: Voucher) => {
    ...
    if (isAuto) {
      showMsg(`Deletion Blocked: System auto-generated voucher ${v.voucherNo} cannot be deleted. Only manual vouchers can be deleted.`, 'error');
      return;
    }
    if (isPeriodLocked(v.date)) {
    ...
    const confirmPrompt = `Are you sure you want to permanently delete manual voucher ${v.voucherNo}? All corresponding General Ledger entries will be removed.`;
                            ...
                            {!isLocked && (
                              <>
                                {!isAuto ? (
                                  ...
                                  <span
                                    className="px-2 py-1 rounded bg-slate-100 text-slate-400 font-bold text-[10px] uppercase tracking-wider border border-slate-200 inline-flex items-center gap-1 select-none"
                                    title="System Auto-Generated Voucher (Protected from deletion)"
                                  >
