      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-4">
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          {baleCategories.map(cat => {
            ...
            const catKey = typeof cat === 'object' && cat !== null ? (cat.id || cat.name) : String(cat || '');
            return (
              <div
                key={catKey}
                  ...
                  <tr key={row.id || row.item_code} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-3 py-2">
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-slate-100 text-slate-700">
                        {typeof row.category === 'object' && row.category !== null ? (row.category as any).name : (row.category || 'Apparel')}
                      </span>
                    </td>
